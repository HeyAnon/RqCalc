...
using RqCalc.Logic.Calc;
using RqCalc.Logic.ImageSource;
using RqCalc.Logic.Serializer.Character;
using RqCalc.Logic.Serializer.GuildTalent;
using RqCalc.Logic.Serializer.Talent;
...
public partial class ApplicationContext : IApplicationContext
{
    ...
    public ApplicationContext(IDataSource<IPersistentDomainObjectBase> dataSource, INativeExpressionParser nativeParser, ISerializer<string, byte[]> urlSerializer, ITypeResolver<string> typeResolver, IImageSourceService imageSourceService)
    {
        ...
        this.LastVersion = this.Settings.CustomLastVersion ?? this.DataSource.GetFullList<IVersion>().OrderBy(ver => ver.Id).Last();

        {
            var characterSerializer = new CharacterSerializer(this);

            this.BinarySerializer = characterSerializer;

            this.Serializer = this.BinarySerializer.Select(urlSerializer);
    ...
    public IVersion LastVersion { get; }
    ...
    public ISerializer<byte[], ICharacterSource> BinarySerializer { get; }

    public ISerializer<string, ICharacterSource> Serializer { get; }
    ...
    internal IStat AttackStat => this.Settings.AttackStat;

    internal IStat DefenseStat => this.Settings.DefenseStat;

    internal IStat HpStat => this.Settings.HpStat;
    ...
    public ICharacterResult Calc(ICharacterSource character)
    {
        ...
        return new CharacterResult
        {
            ...
            Code = this.Serializer.Serialize(character),
    ...
    public int GetFreeTalents(ITalentBuildSource characterInput)
    {
        if (characterInput == null) throw new ArgumentNullException(nameof(characterInput));

        var baseAvailableTalents = Math.Min(characterInput.Level, this.LastVersion.MaxTalentLevel) / this.Settings.TalentLevelMultiplicity;

        var specializationTalents = characterInput.Class.GetSumBonusTalentCount();

        var allAvailableTalents = baseAvailableTalents + specializationTalents;

        var usedTalents = characterInput.Talents.Count;
    ...
    private IReadOnlyList<IReadOnlyList<IStat>> GetDependencyStatLayers(IReadOnlyCollection<IStat> stats)
    {
        ...
        var layers = new { Prev = Enumerable.Empty<IReadOnlyList<IStat>>(), Current = stats.ToList() }.Iterate(pair => pair.Current.Any(), state =>
        {
            ...
            return currentStats.Partial(stat => stat.Sources.All(source => source.Variables.All(v => v.TypeStat == null || state.Prev.Any(layer => layer.Contains(v.TypeStat))))
                                                ...
                                                && !currentDepStat.Any(statPair => statPair.DependencyStats.Contains(stat)), (prev, current) => new { Prev = state.Prev.Concat(new[] { prev }), Current = current });
        });
    ...
    IApplicationSettings IApplicationContext.Settings => this.Settings;
