...
internal partial class CharacterCalc : ICharacterCalc
{
    ...
    public Dictionary<IStat, decimal> GetStats()
    {
        return this.GetBonusEvaluators().GroupBy(evaluator => evaluator.Priority)
            ...
            .Aggregate(new Dictionary<IStat, decimal>(), (startStatsState, startEvalGroup) =>
                ...
                startEvalGroup.SelectMany(evaluator => (evaluator as BonusEvaluator.ConstBonusEvaluator).Maybe(constEvaluator => new[] { constEvaluator })
                                                       ??  (evaluator as BonusEvaluator.DynamicBonusEvaluator).Func(startStatsState))
    ...
    private IEnumerable<BonusEvaluator> GetBonusEvaluators()
    {
        ...
        foreach (var bonus in this.GetBonuses(BonusSource.All).GroupBy(bonus => bonus.Type).SelectMany(g => g.Key.IsSingle ? (IEnumerable<IBonusBase>)new[] { g.FirstOrDefault() } : g))
        {
    ...
    private Dictionary<IStat, decimal> GetEquipmentStats()
    {
        var baseRequest = from equipmentPair in this._character.Equipments
            ...
            from statPair in equipmentInfo.Equipment.Info.Match(info => new[] { new { Stat = this._context.DefenseStat, Value = (decimal)info.Defense } },
                              
                info => new[] { new { Stat = this._context.AttackStat, Value = info.Attack * attackModifier } },
        ...
        var upgradeRequest = from equipmentPair in this._character.Equipments
            ...
            from statPair in upgradeInfo.Match(info => new[] 
                {
                    new { Stat = this._context.DefenseStat, Value = (decimal)info.Defense },  
    ...
    public Dictionary<CharacterEquipmentIdentity, EquipmentUpgradeBaseInfo> GetEquipmentUpgradeInfo()
    {
        var request = from pair in this._character.Equipments
            ...
            let upgradeInfo = equipment.Info.Match<EquipmentUpgradeBaseInfo>(

                info => new EquipmentUpgradeInfo(forgeInfo.AllStatBonus,

                    getDefenseFunc(info.Defense),

                    this._context.EquipmentLevelForges.GetValueOrDefault(Tuple.Create(equipment.Info.InternalLevel, equipmentInfo.Upgrade))),

                info => new WeaponUpgradeInfo(forgeInfo.AllStatBonus, getAttackFunc(info.Attack)),

                info => new DefenseWeaponUpgradeInfo(forgeInfo.AllStatBonus, getAttackFunc(info.Attack), getDefenseFunc(info.Defense)))
