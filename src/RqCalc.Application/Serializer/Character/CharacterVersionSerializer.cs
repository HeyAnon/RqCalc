...
internal class CharacterVersionSerializer
{
    ...
    public CharacterSource Parse(BitReader reader)
    {
        ...
        var collectedItems = this.Version.Collections ? this._collectedGroups[gender].SelectMany(pair => reader.ReadListOptimize(pair.Item2)).ToList() : [];
    ...
    private Dictionary<CharacterEquipmentIdentity, CharacterEquipmentData> ReadEquipments(BitReader reader, IClass @class)
    {
        ...
        var equipmentsRequest = from slotPair in this._slots
            ...
            let cards = baseSlot.IsWeapon == null ? [] : Enumerable.Range(0, maxCardCount).ToList(_ => this.Version.SerializeCardBySlotType ? reader.Read(this._cardsBySlotType[equipment.Type]) : reader.Read(this._cardsBySlot[baseSlot]))
